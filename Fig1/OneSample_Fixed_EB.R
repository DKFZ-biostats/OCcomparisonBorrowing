...
########################################################################
########################################################################
### 1 sample, borrowing with Empirical Bayes power prior, external data is fixed
###########################
rm(list = ls())
library(tidyverse)
library(parallel)
  ...
  results_inc[[i]] <- as.data.frame(t(simplify2array(
    mclapply(1:nsim, 
             function(my.i) {
               ...
               alphaB <-
                 PowerFix(
                   pprior = pPowerpriorEB, # type I error rate when borrowing from this historical data set
               ...
               # frequentist power for adjusted alpha
               powerwo_alphaB <- pnorm(sqrt(N) * (theta1 - H0MU) / SIGMA - qnorm(1 - alphaB))  
               
               #compare power for w and w/o borrowing calibrated to typeI error rate with borrowing
...
resultsEB <- bind_rows(results_inc)
names(resultsEB) <-
  c("thetaE",
    "mean.histdata",
    "alphaB",
    ...
    "powerwEBdiff")
write.csv(resultsEB, 
          file = "OneSampleEB_delta0.5_alpha0.025_nE20.csv")
